...
call IMAP ("TFOP","for p := 0; p < <++>; p++ {\<cr><++>\<cr>}","go")
call IMAP ("TFOQ","for q := 0; q < <++>; q++ {\<cr><++>\<cr>}","go")
...
call IMAP ("TPLA","plt.Reset(true, &plt.A{WidthPt: 400, Dpi: 150})\<cr>plt.Plot(<++>, <++>, &plt.A{C: \"b\", Ls: \"-\", L: \"curve1\", NoClip: true})\<cr>plt.Gll(\"$x$\", \"$y$\", nil)\<cr>plt.HideTRborders()\<cr>plt.Save(\"/tmp<++>\", \"<++>\")\<cr>","go")
call IMAP ("TPLB","xx := utl.LinSpace(<++>, <++>, <++>)\<cr>yy := utl.GetMapped(xx, func(x float64) float64 { return <++> })\<cr>plt.Reset(true, nil)\<cr>plt.Plot(xx, yy, &plt.A{C: \"r\", NoClip: true})\<cr>plt.Gll(\"$x$\", \"$y$\", nil)\<cr>plt.HideTRborders()\<cr>plt.Save(\"/tmp<++>\", \"<++>\")\<cr>","go")
