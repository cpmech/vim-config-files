...
call IMAP ("TPF","io.Pf(\"<++> = %v\\n\", <++>)","go")
call IMAP ("TQF","io.Pforan(\"<++> = %v\\n\", <++>)","go")
call IMAP ("TRF","io.Pfblue2(\"<++> = %v\\n\", <++>)","go")
...
call IMAP ("TPLA","plt.Reset(true, &plt.A{WidthPt: 400, Dpi: 150})\<cr>plt.Plot(X, Y, &plt.A{C: \"b\", Ls: \"-\", L: \"curve1\", NoClip: true})\<cr>plt.Gll(\"x\", \"y\", nil)\<cr>plt.HideTRborders()\<cr>plt.Save(\"/tmp\", \"figure01\")","go")
call IMAP ("TPLB","X := utl.LinSpace(<++>, <++>, <++>)\<cr>Y := utl.GetMapped(X, func(x float64) float64 { return <++> })\<cr>plt.Reset(true, nil)\<cr>plt.Plot(X, Y, &plt.A{C: \"r\", NoClip: true})\<cr>plt.Gll(\"x\", \"y\", nil)\<cr>plt.HideTRborders()\<cr>plt.Save(\"/tmp\", \"figure01\")","go")
